...
window.PARAMS = {
  ...
  orbit: {                // 第2章：太阳的椭圆轨道（带俯角：横向长、纵向扁，像从稍高处俯视一个水平圆环）
                          // 椭圆的大小、虚线的粗细与不透明度在 theme.js 的 STYLE 里（orbitWidthRatio / orbitAspect / orbitStroke …）
    moonYRatio: 0.44,     // 主月亮中心离画布顶部的比例（第2章没有地面，月亮可以更居中）
    grabBand: 130,        // 手指离轨道多远以内算「抓住了轨道」（画布单位）
    centerDeadZone: 120   // 手指离月心太近时忽略（画布单位；方位角会抖）
  },

  guide: {                // 第2章：判定与三步引导（提词见 chapter2.js 的 STEPS）
    toleranceDeg: 10,     // 角度目标（正前方 / 正后方 / 「一个月」的各个相位）允许的误差（±）
    stillDegPerSec: 15,   // 太阳角速度低于多少（度/秒）算「手指基本停住」——停住了才开始 1 秒倒计时，拖动中不计时
    speedWindowMs: 250,   // 估角速度的时间窗：最近这么多毫秒里太阳角度的摆动范围（最大 − 最小）÷ 时间
    gapMin: 0.25,         // 第二步：月亮缺掉的部分至少占多少算「明显缺口」
    holdMs: 1000,         // 停住后在目标区域里停多久算完成（月亮外围的细弧线在这段时间里填满）
  ...
  month: {                // 第2章「一个月」回合（三步引导之后；引导只在首次出现，之后直接从这里开始）
                          // 顶部正中的「目标月相 + 环绕的 8 颗进度点」部件的尺寸在 theme.js 的 STYLE 里（goalMoonR / goalRingR / dot*）
    prompt: '下一个是这样的月亮',        // 八个目标共用的一句提词（目标月相画在顶部部件的中央）
