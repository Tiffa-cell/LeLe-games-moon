 ...
 *
 * 章节入口（不做菜单、不做文字）：
 *   第1章过关后（没过关不显示）、镜头拉回时，画面底部正中出现一个太阳图标 + 右侧一个「→」，点它进第2章；
 *   第2章过关后，左上角出现一个「←」+ 小月亮，点它回第1章。
 *   箭头是细线、圆头、略带手绘感、低不透明度，带呼吸动画（线宽、不透明度在 theme.js 的 STYLE 里）。
...
(function () {
  ...
  function navArrow(dir) {                       // 「→」/「←」：细线、圆头、略带手绘感（杆微微起伏、两翼不完全对称），呼吸动画（CSS）
    var s = svgEl('svg', { viewBox: '0 0 40 40', 'aria-hidden': 'true', 'class': 'nav-arrow', 'data-dir': dir });
    var d = dir === 'right'
      ? 'M 7.5,20.8 C 14,19.4 22.5,21.2 31.6,19.9 M 23,11.6 C 25.8,15.3 28.6,17.9 31.6,19.9 C 28.4,22.4 25.4,25.5 22.4,28.9'
      : 'M 32.5,19.2 C 26,20.6 17.5,18.8 8.4,20.1 M 17,11.1 C 14.2,14.8 11.4,17.4 8.4,20.1 C 11.6,22.6 14.6,25.7 17.6,29.1';
