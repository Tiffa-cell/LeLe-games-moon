...
- 环上有一颗**发着光的小月亮**：拖着它沿环走（在环的任何位置按住拖也可以）。
  中央的大月亮实时跟着变；松手时会轻轻吸附到最近的相位；经过每个相位小月亮时它会轻轻一跳。
- 走完一整圈、回到起点的空心圆就过关：**小人抬头，星星从他身边开始一颗一颗亮起来，镜头回到他身上**。
  停一会儿（或点一下屏幕）镜头再拉远，可以继续一圈一圈玩。
- 右上角三个圆点切换夜色：墨青 / 黛蓝（默认）/ 暗紫。选择会记住。
- 进度存在浏览器的 localStorage 里：过关后星星以后一直亮着。想重置就清除这个网站的数据，没有账户。
...
| `stars` | `count` / `sparkles` | 星点数 / 四角星数（按画布面积缩放） | 46 / 6 |
| | `dim` | 未过关时星星亮度系数 | 0.3 |
| | `lightMs` / `staggerMs` | 每颗点亮时长 / 逐颗间隔 | 600 / 55 |
| | `seed` | 星空随机种子 | 42 |
| `camera` | `introHoldMs` / `introZoomMs` | 开场停在小人身上 / 拉远时长 | 900 / 2600 |
| | `winDelayMs` / `winZoomMs` | 过关后镜头等多久 / 回到小人的时长 | 1400 / 2600 |
| | `holdMs` / `returnZoomMs` | 停在小人身上多久 / 拉回宇宙视角的时长 | 3500 / 1800 |
...
- **测试**（需要 Playwright，仅开发用）：
  ...
  会在 `tools/shots/` 留下各个阶段的截图（iPad 竖屏 / 横屏 / iPhone、拖一圈过关、三套夜色）。
